...
def preprocess_macro_factors(bc_df, annual_df, ir):
    ...
    for col in bc_renamed.columns:
        if col.startswith(('GDP_', 'CPI_')):
            ...
            # Diff here, previously in bc portfolio construction
            bc_renamed[col] = bc_renamed[col].diff()
    ...
    for country in countries:
        ...
        risk_free_rate = (1 + ir[f'Interest Rates_{country_to_abbr[country]}']/100) ** (1/252) - 1
...
def calculate_bc_momentum(mf_df):
    ...
    gdp_changes = mf_df.filter(like='GDP_')
    cpi_changes = mf_df.filter(like='CPI_')
...
def construct_bc_portfolio(ar_df, mf_df, vol=0.01):
    """
    ...
    equity_weights = create_asset_class_portfolio_bc(ar_df, bc_scores, 'Equity', target_vol=vol)
    fx_weights = create_asset_class_portfolio_bc(ar_df, bc_scores, 'FX', target_vol=vol)
    bond10y_weights = create_asset_class_portfolio_bc(ar_df, bc_scores, 'Bond10Y', target_vol=vol)
    ir_weights = create_asset_class_portfolio_bc(ar_df, bc_scores, 'IR', target_vol=vol)
...
def construct_it_portfolio(ar_df, mf_df, vol=0.01):
    """
    ...
    # Create portfolios for each asset class
    equity_weights = create_asset_class_portfolio_it(ar_df, it_scores, 'Equity', target_vol = vol)
    fx_weights = create_asset_class_portfolio_it(ar_df, it_scores, 'FX', target_vol = vol)
    bond10y_weights = create_asset_class_portfolio_it(ar_df, it_scores, 'Bond10Y', target_vol = vol)
    ir_weights = create_asset_class_portfolio_it(ar_df, it_scores, 'IR', target_vol=vol)
...
def construct_mp_portfolio(ar_df, mf_df, vol=0.01):
    """
    Construct complete Business Cycle long-short portfolio across all asset classes
    """
    # Calculate BC momentum scores
    ...
    # Create portfolios for each asset class
    equity_weights = create_asset_class_portfolio_mp(ar_df, mp_scores, 'Equity', target_vol = vol)
    fx_weights = create_asset_class_portfolio_mp(ar_df, mp_scores, 'FX', target_vol = vol)
    bond10y_weights = create_asset_class_portfolio_mp(ar_df, mp_scores, 'Bond10Y', target_vol = vol)
    ir_weights = create_asset_class_portfolio_mp(ar_df, mp_scores, 'IR', target_vol=vol)
...
def construct_rs_portfolio(ar_df, mf_df, vol=0.01):
    """
    Construct complete Business Cycle long-short portfolio across all asset classes
    """
    # Calculate BC momentum scores
    ...
    # Create portfolios for each asset class
    equity_weights = create_asset_class_portfolio_rs(ar_df, rs_scores, 'Equity', target_vol = vol)
    fx_weights = create_asset_class_portfolio_rs(ar_df, rs_scores, 'FX', target_vol = vol)
    bond10y_weights = create_asset_class_portfolio_rs(ar_df, rs_scores, 'Bond10Y', target_vol = vol)
    ir_weights = create_asset_class_portfolio_rs(ar_df, rs_scores, 'IR', target_vol=vol)
...
def create_asset_class_portfolio_mp(asset_returns, mp_scores, asset_class, target_vol=0.01):
    ...
    if asset_class == 'Equity':
        ...
        assets = [col for col in asset_returns.columns if col.startswith('Interest Rate_')]
        countries = [col.split('_')[1] for col in assets]
        # IR: Yield up -> bullish for literally interst rate itself
        raw_scores = -mp_scores[countries]
...
def create_asset_class_portfolio_it(asset_returns, it_scores, asset_class, target_vol=0.01):
    ...
    if asset_class == 'Equity':
        ...
        assets = [col for col in asset_returns.columns if col.startswith('FXReturn_')]
        countries = [col.split('_')[1] for col in assets]
        # FX: Depre -> more expensive import -> high int rate for  -> bullish for fx
        raw_scores = it_scores[countries]

    elif asset_class == 'IR':
        assets = [col for col in asset_returns.columns if col.startswith('Interest Rate_')]
        countries = [col.split('_')[1] for col in assets]
        # IR: Depre -> more expensive import -> high int rate to press down inflation -> bullish for fx
        raw_scores = it_scores[countries]
...
def create_asset_class_portfolio_bc(asset_returns, bc_scores, asset_class, target_vol=0.01):
    ...
    if len(assets) > 1:
        ...
        # Resample to daily and ffill on weights
        #weights = weights.resample('D').ffill()
...
def integrate_portfolios(bc_portfolio, it_portfolio, mp_portfolio, rs_portfolio):
    portfolios = [bc_portfolio, it_portfolio, mp_portfolio, rs_portfolio]
    
    # === Combine 'weights' and 'returns' ===
    final_weights = portfolios[0]['weights'].copy(deep=True)/4
    final_returns = portfolios[0]['returns'].copy(deep=True)/4
    
    for p in portfolios[1:]:
        final_weights += p['weights']/4
        final_returns += p['returns']/4
    ...
    for key in component_keys:
        ...
        combined_df = portfolios[0]['components'][key].copy(deep=True)
        # Add the others
        for p in portfolios[1:]:
